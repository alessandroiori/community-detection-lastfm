...
delimiter = ' '
path_file = "file:///files/cosim-sorted-row-num.6.2plexes"
...
def run():
    ...
    q = 'LOAD CSV FROM {file} AS line FIELDTERMINATOR ":" ' \
        ...
        'WHERE NOT row_count IN u.2plexes ' \
        'SET u.two_plexes = u.two_plexes + row_count ' \
        ')' \
    ...
    result = session.run(q1, {"file": path_file})
    for r in result:
