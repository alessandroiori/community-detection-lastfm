...
def NodesMinusNodes(nodes,nodes1,nodes2):
    return (item for item in nodes if item not in chain(nodes1,nodes2))
...
def run(G, num_of_kplex, neighbors, output_file):
    ...
    #RecursiveGen2(G,nodes, 0,[],num_of_kplex)
    end = time.time()
...
def RecursiveGen(G,nodes, G_curr1, i,E1,num_of_kplex, output_file):
    ...
    while stack.__len__()>0 :
        ...
        for u in NodesMinusNodes(nodes, G_curr[:], E):
            for H,ng_dic in EnumAlmostSat(G, G_curr[:],u,my_copy(local_ng_dic)):
                ...
                if(h_s not in clique_set):
                    ...
                    if end - start > 20:
                        print "Running time for enumerating %d k-plexes: %f" % (clique_set.__len__(),(end - start))
                        return
                    if(clique_set.__len__()>=num_of_kplex):
                       return
